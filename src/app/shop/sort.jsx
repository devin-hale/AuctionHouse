...
export const Sort = ({ sort, setSort }) => {
  ...
  return (
    <div className="w-[100%] flex flex-row flex-nowrap justify-start">
      <div className={`${isMobile ? `` : `w-[20%]`}`}></div>
      <div
        className={`${
          isMobile ? `w-[100%] flex flex-row justify-center` : `w-[20%]`
        } drop-shadow-[0px_0px_3px_rgba(0,0,0,0.4)] `}
      >
        <input
          className={`pl-1 rounded ${isMobile ? `w-[60%]` : null}`}
          type="text"
          placeholder={`Search`}
          maxLength={25}
      ...
      {isMobile && (
        <div
          className={`p-2 m-2 rounded drop-shadow-[0_2px_5px_rgba(0,0,0,.25)] transition-all bg-white fixed top-0 right-0 flex flex-col items-end ${
